var store = [{
        ...
        "title": "자료구조",
        "excerpt":"시간복잡도 알고리즘 효율성을 판단하는 지표로써, 알고리즘 수행에 걸리는 시간이 아닌 연산들이 몇번 이루어지는가 에 대한 지표이다. 메모리 사용량을 분석한 결과인 공간복잡도 라는것도 있다. Big-O 표기법 - 시간복잡도 빠른 순서 O(1) : 데이터의 크기에 상관없이 언제나 일정한 시간이 걸림 (index를 통한 접근) O(log₂ n) : 데이터가 커질수록 시간이 로그만큼 짧아짐 (이진탐색...","categories": ["Data_Structure"],
        "tags": ["data structure"],
